...
This document tracks the current completion state of the MiroFish Gold Trading project.
...
- **Smart Trailing Stop:** A robust `ManageTrailingStop()` function is coded directly into the `MiroFishEA.mq5` file. Once a trade clears +35 pips in profit, MT5 automatically trails the price by 15 pips to lock in profits without needing the Python server.
...
- **Phase 3 (Full MiroFish AI Mode):** Testing `--engine mirofish`, where the LLM is given complete autonomy over market analysis, risk sizing, and trade selection.
